...
## RUNPOD GPU RUN PLAN + turnkey package (PLAN ONLY, no GPU/training) — 2026-06
- Client approved: RTX 4090 24GB, RunPod Secure Cloud, ~3-4h, ~$2-3, hard cap $50. Real run = SegFormer-B0 AND B1 @768 binary, same dataset/split/schedule/eval.
- Package /app/gpu_run/: requirements-gpu.txt, export_dataset_bundle.py, preflight.py, train_gpu.py, export_onnx.py, run.sh, collect_and_checksum.sh + dataset_bundle/ (573MB). Plan: reports/milestone1_v2_gpu_run_plan/RUNPOD_EXECUTION_PLAN.md.
- Frozen bundle built (read-only): 150 approved, holdout=FIXED 13, pool_train=137, 0 group-overlap; bundle_sha256 ca19cd07...65eb1. Preflight PASS locally (holdout==13, no leak, checksums verified).
- Methodology locked = train_step1 (AdamW 6e-5/wd1e-4, weighted CE [1,3], seed0, 18 ep, bs4, 768). ONE flagged client-directed change (#9): checkpoint selection by MIN mean perpendicular roofline error (was Dice). train_gpu does single-pass + hflip-avg eval, saves raw logits+probs for all holdout, exports ONNX opset17 static [1,3,768,768]->[1,2,768,768] in-graph upsample, for BOTH b0 and b1.
- NO git remote → code transfer via Emergent Save-to-GitHub then clone, or runpodctl send. Data via runpodctl/cloud (bundle self-contained; NO Mongo/storage creds on pod).
- Recalculated estimate: B0 ~6-10min, B1 ~18-28min, +eval/export ~3-5min; total Pod ~1h (budget 1.5h). Cost Secure $0.74/hr → ~$0.75-1.10 expected, <$3 conservative, hard cap $50. NO GPU rented, NO training started, holdout/annotations untouched.
